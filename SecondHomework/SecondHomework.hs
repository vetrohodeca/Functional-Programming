...
getAverageBalance (acc , persons) predicate =
    (foldr (+) 0-- sumata ot vs balansi 
        (map
            (\ (_,_,x) -> x)
            (filter
                (checkAccount (filter predicate persons))--vrushta vsichki personi, koito izpulnqvat predikata
                acc
            )
        )
    )
    ...
    (fromIntegral
        (length
            (filter
                (checkAccount (filter predicate persons ))
                acc
            )
        )
    )
 
checkAccount:: [Person] ->Account -> Bool
checkAccount persons (_,personID,_) =
    if (length (filter (\ (x,_,_) -> x == personID) persons)) > 0
            then True
            else False